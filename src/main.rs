...
fn df_to_json(df: DataFrame) -> Result<Json<Value>, StatusCode> {
    serde_json::to_value(&df)
        .map_err(|e| {
            error!("can't serialize dataframe to json: {}", e);
...
fn cities_zip_to(df_cities: &DataFrame, zip: &str, field: &str) -> Result<DataFrame, StatusCode> {
    df_cities
        ...
        .collect()
        .map_err(|e| {
            error!("couldn't query {} for zip: {}", field, e);
...
async fn delays_by_zip(
    ...
    let delays_for_zip = app_state
        ...
        .join(
            ...
            [col("zip")],
            [col("zip")],
            JoinArgs::new(polars::prelude::JoinType::Inner),
        )
        .group_by(["zip"])
        .agg([
            len().alias("len"),
        ...
        .collect()
        .map_err(|e| {
            error!("couldn't query delay by zip {}: {}", zip, e);
            StatusCode::BAD_REQUEST
        })?;
...
async fn delays(
    ...
    let delays = app_state
        ...
        .collect()
        .map_err(|e| {
            error!("can't query delay times: {}", e);
            StatusCode::BAD_REQUEST
        })?;
    df_to_json(delays)
}
...
async fn delays_corr(
    ...
    let corr = app_state
        ...
        .map_err(|e| {
            error!(
                "couldn't query delay time correlation between {} and {}: {}",
                field_1, field_2, e
            );
            StatusCode::BAD_REQUEST
        })?;
...
async fn main() {
    ...
    let df_delays = CsvReadOptions::default()
        ...
        .expect("can't read delays csv")
        .finish()
        .expect("can't create delays dataframe");

    let df_cities = CsvReadOptions::default()
        .with_has_header(true)
        .try_into_reader_with_file_path(Some("zip_pop.csv".into()))
        .expect("can't read cities csv")
        .finish()
        .expect("can't reading cities dataframe");
    ...
    let listener = TcpListener::bind("0.0.0.0:3333")
        .await
        .expect("csn't start web server on port 3333");
    info!("listening on {:?}", listener.local_addr());
    axum::serve(listener, app)
        .await
        .expect("can't start web server");
}
